...
styles = [
    ...
    {"name": "漫画", "prompt": "{prompt}, anime --niji 5", "negative_prompt": ""},
    {
        "name": "电影",
        "prompt": "photorealistic, cinematic, shot on kodak detailed cinematic hbo",
        "negative_prompt": "",
    },
    {"name": "水墨画", "prompt": "{prompt}, chinese ink painting", "negative_prompt": ""},
    {"name": "油画", "prompt": "{prompt}, oil painting", "negative_prompt": ""},
    {"name": "水彩画", "prompt": "{prompt}, watercolor painting", "negative_prompt": ""},
    {"name": "铅笔画", "prompt": "{prompt}, pencil drawing", "negative_prompt": ""},
    {"name": "写实", "prompt": "{prompt}, realistic, depth of field", "negative_prompt": ""},
]
...
async def apply_style(style, positive, negative):
    if style in styles.keys():
        p, n = styles.get(style, default_style)
        p = p.replace("{prompt}", positive)
    else:
        postfix = await translator(style)
        p = f"{positive}, {postfix}"
        n = ""
    no_parts = [n, negative]
    filtered_no_parts = [part for part in no_parts if part]
    if filtered_no_parts:
        prompt = f"{p} --no {', '.join(filtered_no_parts)}"
    else:
        prompt = p
    return prompt


# style = "photo-tilt-shift"
# positive = "a girl"
# negative = ""

# p, n = apply_style(style, positive, negative)
