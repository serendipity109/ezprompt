        ...
        <div class="flex flex-wrap overflow-hidden flex-1 items-start justify-start thin-scrollbar h-auto pb-2"
          style="height:fit-content;overscroll-behavior-x:contain">
            <div v-for="(img_url, index) in img_res" :key="index">
              <img v-bind:src="img_url" v-on:click="showViewer(img_res)" style="object-fit:contain;height:100%;max-height:50vh" />
            </div>
          </div>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchImageData = async (img) => {
      const response = await axios.get(`http://${process.env.VUE_APP_BACKEND_IP}/get_image`,
        {
          params: {
            img: img,
          }
      ...
      img_res.value = res.img_res;
      const pmt_res = res.pmt_res;
      pmt.value = pmt_res.prompt;
      npmt.value = pmt_res.nprompt;
      dims.value = pmt_res.dims;
      CFG.value = pmt_res.CFG;
    };

    fetchImageData(route.params.img);
